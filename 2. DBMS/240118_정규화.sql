-- 정규화
-- > 데이터베이스의 테이블을 올바른 형태로 개선해 나가는 것
...
-- > 주문 상품의 테이블에서는 주문 번호와 상품코드를 하나로 묶어 기본키로 지정할 수 있음
...
-- 이론적으로는 제5 정규형까지 있지만 프로젝트 구조나 상황에 다라서 정규화 단계를 선택할 수 있음
-- > 일반적으로는 제3 정규형까지를 채택
 

-- 정규화의 목적
-- > 정규화에서는 중복되거나 반복되는 부분을 찾아내서 테이블을 분할하고 기본키를 작성해 사용하는 것을 기본 개념으로 삼음
-- > 정규화의 모든 과정은 '하나의 데이터는 한 곳에 있어야 한다'는 규치겡 근거함
