...
-- -- 예) 색인에 만들지 않아도 될 단어들이 쓸데 없이 쌓여있으면 책의 두께만 두꺼워지고 색인을 사용하더라도 색인을 사용하지 않을 때보다 더 오래 걸릴수도 있음
...
-- > 책에 색인을 만든다고 해서 책의 본문이 변경되는 것은 아닌 것 처럼 보조 인덱스를 만든다고 해서 데이터의 순서나 내용이
-- > 바뀌지는 않음
DROP TABLE IF EXISTS buy, member;
CREATE TABLE member
( mem_id			CHAR(8),
...
ALTER TABLE member
	...
	UNIQUE (mem_name);
SELECT * FROM member;
-- > 데이터의 내용과 ㅅ누서는 그대로이며, mem_id열과 mem_name열에 모두 보조 인덱스가 생성된 상태임
...
-- 인덱스의 내부 작동 원리
-- > 인덱스가 늘 좋은 것은 아니므로 인덱스의 내부 작동 원리를 이해하고 인덱스를 사용해야할 경우와 사용하지 말아야 할 경우
-- > 사용하지 말아야 할 경우를 정확히 판단하는 것이 중요

-- 균형 트리의 개념
-- > 군형 트리 구조에서 데이터가 저장되는 공간을 노드(node)라고 함
